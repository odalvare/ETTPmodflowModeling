...
modelname,Lx,Ly,Lz,zbot,nlay,delcx,delry,nper,steps,grid,wlay=rd.readdata()
...
#    
#nper=20                                                 #Number of periods of simulation
#perlen=np.ones(nper, dtype=np.float32)
#perlen=[1000./float(nper) for x in perlen]              #Stress period duration in days
##perlen=[x*86400 for x in perlen] #                                      en segundos (SI)
#nstp=np.ones(nper, dtype=np.int32)
#for i in range(0,nper,1):                               #Number of time steps in each stress period
#    nstp[i]=5
#steady=False
...
dis = flopy.modflow.ModflowDis(mf, nlay, nrow, ncol, delr=delry, delc=delcx,
                               top=ztop, botm=botm[1:], nper=nper, perlen=perlen, 
                               nstp=nstp, steady=False, itmuni=4, lenuni=2)
...
ibound[0, :, 0] = -1
ibound[0, :, ncol-1] = -1   #This line will change
strt = np.ones((nlay, nrow, ncol), dtype=np.float32)
strt[0, :, 0] = 2.
strt[0, :, ncol-1] = 0.     #This line will change
...
pumping_rate = 1000.
wcol = round(ncol/10) # x index for the well
wrow = round(nrow/2)  # y index for the well
wel_sp = {}
wel_sp[0] = [ (0, wrow, wcol, 0.) ]
print(wel_sp)
for i in range(1,nper,1):
    if i==4:
        ...
        wel_sp[i]=[(0, wrow, wcol, pumping_rate)]    
    else:
        wel_sp[i]=[(0, wrow, wcol, 0.)]
...
sg=Sgemsimp(dataspace,'NSU_real',27,'Trans')    #101 with 50 #305:reference
...
hkd=sg.readsgems(ncol,nrow,nlay)
for k in range(0,nlay,1):
    for i in range(0,nrow,1):
        for j in range(0,ncol,1):
            if hkd[k,i,j]<0.01:
                print('----- Invalid number found -----')
                print('before filtering='+str(hkd[k,i,j]))
                print('after filtering='+str(0.01))
                hkd[k,i,j]=0.01
hkd=hkd/20.0    
vkd = hkd
...
# dispersion package
dsp = flopy.mt3d.Mt3dDsp(mt, al=10, trpt=0.1, trpv=0.1, dmcoef=1e-09)
...
for i in range(1,nper,1):
    if i==4:
        #wel_sp[0].append((0, wrow, wcol, pumping_rate)) # lay, row, col index, pumping rate
        #append is used for adding information on each row of the library
        ssm_data[i]=[(0, wrow, wcol, 100.0, itype['WEL'])]    
    else:
